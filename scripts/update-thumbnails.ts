...
for (const entry of entries) {
  ...
  // Detectar modo
  const mode = normalizedEntry.includes("dark")
    ? "dark"
    : normalizedEntry.includes("light")
      ? "light"
      : null;
  if (!mode) continue;
  ...
  for (const key in componentMapping) {
    ...
    if (
      normalizedEntry.includes(key) ||
      normalizedEntry.includes(key.normalize("NFC")) ||
      normalizedNFD.includes(key.normalize("NFD"))
    ) {
